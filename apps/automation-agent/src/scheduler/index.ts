...
export class SchedulerService {
  ...
  async start(): Promise<void> {
    ...
    this.scheduleJob(
      "fee-reminder-due-check",
      "0 6 * * *",
      this.checkFeeRemindersDue.bind(this)
    );
   ...
   */
  private async checkFeeRemindersDue(): Promise<void> {
    try {
      ...
      const students = await db
        ...
        .where(
          and(
            ...
            lte(studentsTable.nextFeeDueDate, todayStr),
            gte(studentsTable.nextFeeDueDate, sql`${todayStr}::date - interval '3 days'`),
          )
        );

      for (const student of students) {
        if (!student.nextFeeDueDate) continue;

        try {
          // Trigger fee reminder event for this student
          await eventService.triggerFeeReminderDue(
            student.id,
            student.id, // Using student ID as fee record reference
      ...
      const records = await db
        ...
        .where(
          and(
            ...
            lte(feeRecordsTable.dueDate, today),
          )
        );
    ...
    try {
      ...
      const students = await db
        ...
        .where(
          and(
            eq(studentsTable.isArchived, false),
            lt(attendanceSummaryTable.attendancePercentage, 75),
          )
        );
