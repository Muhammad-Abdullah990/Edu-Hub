...
import { getStudentBySlug } from "@/data/studentsData";
...
export default function StudentProfile({ slug }: StudentProfileProps) {
  ...
  return (
    <>
      <Helmet>
        <title>{student ? `Student Success – ${student.name} | Toppers Coaching Center` : "Student Profile | Toppers Coaching Center"}</title>
        <meta name="description" content={student ? `${student.name} from ${student.classLevel} improved with ${student.yearsWithInstitute} years at Toppers Coaching Center.` : "View current student journeys at Toppers Coaching Center."} />
        <link rel="canonical" href={`https://topperscoachingcenter.com/student/${slug}`} />
        <meta property="og:title" content={student ? `Student Success – ${student.name} | Toppers Coaching Center` : "Student Profile | Toppers Coaching Center"} />
        <meta property="og:description" content={student ? `${student.name} from ${student.classLevel} improved with ${student.yearsWithInstitute} years at Toppers Coaching Center.` : "View current student journeys at Toppers Coaching Center."} />
          ...
          <section className="bg-white rounded-[36px] border border-slate-100 shadow-xl shadow-slate-200/50 p-8 md:p-12 mb-10 relative overflow-hidden">
            ...
            <div className="relative z-10 grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
              <div>
                ...
                <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">{student.name}</h1>
                <p className="text-xl text-slate-600 mb-6">
                  {student.classLevel} • With us for {student.yearsWithInstitute} {student.yearsWithInstitute === 1 ? "year" : "years"}
                </p>
                <div className="flex flex-wrap gap-3 mb-6">
                  <span className="px-4 py-2 rounded-full bg-accent/15 text-amber-700 font-semibold text-sm">{student.badge}</span>
                  <span className="px-4 py-2 rounded-full bg-success/10 text-emerald-700 font-semibold text-sm">{student.shortHighlight}</span>
                  {student.rankOrPosition && (
                    <span className="px-4 py-2 rounded-full bg-secondary/10 text-secondary font-semibold text-sm">{student.rankOrPosition}</span>
                  )}
                </div>
                <p className="text-lg text-slate-600 max-w-2xl">
                  This profile reflects the kind of continuity, support, and measurable progress families want when choosing a coaching center for the long term.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600">{student.classLevel}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600">{student.schoolName}</p>
                    </div>
                  </div>
                  {student.residenceArea && (
                    <div className="flex items-start gap-3">
                      <MapPin className="w-5 h-5 text-primary mt-0.5 shrink-0" />
                      <div>
                        <p className="font-semibold text-slate-900">Area</p>
                        <p className="text-slate-600">{student.residenceArea}</p>
                      </div>
                    </div>
                  )}
                  {student.fatherName && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-900">Father Name</p>
                        <p className="text-slate-600">{student.fatherName}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-8">
            <motion.section initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="space-y-8">
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                ...
                <p className="text-slate-600 leading-8 text-lg">{student.detailedStory}</p>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                ...
                <div className="space-y-6 relative before:absolute before:left-[19px] before:top-3 before:bottom-3 before:w-px before:bg-slate-200">
                  {student.resultsTimeline.map((entry, index) => (
                    <div key={`${student.id}-${entry.year}`} className="relative flex gap-5">
            ...
            <motion.aside initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="space-y-8">
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                ...
                <ul className="space-y-4">
                  <li className="rounded-2xl bg-accent/10 border border-accent/20 px-4 py-4 font-medium text-slate-700">{student.shortHighlight}</li>
                  <li className="rounded-2xl bg-primary/8 border border-primary/15 px-4 py-4 font-medium text-slate-700">
                    Stayed with Toppers for {student.yearsWithInstitute} {student.yearsWithInstitute === 1 ? "year" : "years"}
                  </li>
                  {student.rankOrPosition && (
                    <li className="rounded-2xl bg-secondary/10 border border-secondary/15 px-4 py-4 font-medium text-slate-700">{student.rankOrPosition}</li>
                  )}
                </ul>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                ...
                <div className="space-y-5">
                  <div>
                    <p className="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-1">School</p>
                    <p className="text-slate-800 font-medium">{student.schoolName}</p>
                  </div>
                  <div>
                    <p className="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-1">Class Level</p>
                    <p className="text-slate-800 font-medium">{student.classLevel}</p>
                  </div>
                  <div>
                    <p className="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-1">Retention</p>
                    <p className="text-slate-800 font-medium">{student.yearsWithInstitute} year academic relationship</p>
                  </div>
                </div>
              </div>
            </motion.aside>
